...
def _get_cg_solver(
    ...
    if parallel_iterations and show_progress:
        raise ValueError("Can only use one of `parallel_iterations` or `show_progress`")

    if parallel_iterations:
        solver = MappedMatrix(solver, parallel_iterations=parallel_iterations)
    if show_progress:
...
def power_series_shifted_laplacian_inverse(
    ...
    st = st * (1 - epsilon)
    matrix = StaticPowerSeriesMatrix(
        SparseMatrix(st), [epsilon if rescaled else 1] * num_terms
    )
...
def get_pseudo_inverse_propagators(
    ...
    *,
    ...
    w += 2
    eigengap = tf.convert_to_tensor(w[0], L.dtype)
    tf.debugging.assert_all_finite(eigengap, "eigengap must be finite")
    ...
    base = CGSolverMatrix(
        # SumMatrix(
        #     (L, -u0m @ u0m_adjoint), is_self_adjoint=True, is_positive_definite=True
        # ),
        L,
        tol=tol,
        max_iter=max_iter,
    ...
    projector = (
        ScaledIdentityMatrix(num_nodes, tf.ones((), dtype=base.dtype))
        - u0m @ u0m_adjoint
    )
    base = CompositionMatrix((base, projector))
    rows = GatherMatrix(row_ids, L.shape[0])
    if preprocess:
        if show_progress:
            base = ProgMatrix(base)
        base = rows.to_dense() @ base
        if rescaled:
            base = base * eigengap
    else:
        factors = [rows, base]
        if rescaled:
            factors.insert(0, ScaledIdentityMatrix(rows.shape[0], eigengap))
        base = CompositionMatrix(tuple(factors))
    # base = sparse_cg_solver(
    #     SumMatrix(
    #         (L, -u0m @ u0m_adjoint), is_self_adjoint=True, is_positive_definite=True
    #     ),
    #     row_ids,
    ...
    # base = base @ projector
    if include_uut:
        u0_gathered = tf.gather(u0, row_ids)
        if not rescaled:
            u0_gathered = u0_gathered / eigengap
            u0_gathered = u0_gathered / 10  # HACK
        if preprocess_uut:
            uut = u0_gathered @ tf.transpose(u0)
        else:
