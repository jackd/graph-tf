...
class GATConv(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.attn_dense = MultiHeadedDense(
            ...
            kernel_initializer=rescale_initializer(attn_kernel_initializer, 2),
    ...
    def call(self, inputs, training=None):
        ...
        q, k = tf.unstack(self.attn_dense(x), axis=-1)

        row, col = tf.unstack(a.indices, axis=-1)
        attn = tf.nn.leaky_relu(tf.gather(q, row, axis=0) + tf.gather(k, col, axis=0))
